...
class AuthController {
    static login = async (req, res) => {
        ...
        try {
            ...
            const accessToken = jsonwebtoken_1.default.sign({ userId: user.id }, auth_config_1.default.secret, // Use the secret from the authConfig for signing the access token
            { expiresIn: auth_config_1.default.secret_expires_in } // Use the expiration time from the config (e.g., "15m")
            );
            // 4. Generate a refresh token with a longer expiration time (e.g., 7 days)
            const refreshToken = jsonwebtoken_1.default.sign({ userId: user.id, }, auth_config_1.default.refresh_secret, // Use the separate secret for signing the refresh token
            { expiresIn: auth_config_1.default.refresh_secret_expires_in } // Use the expiration time for the refresh token (e.g., "24h")
            );
    ...
    static register = async (req, res) => {
        ...
        try {
            ...
            if (existingUser) {
                return response_utils_1.default.error(res, null, "Email is already in use.");
            }
    ...
    static refreshToken = async (req, res) => {
        try {
            ...
            return response_utils_1.default.success(res, { message: "Access token refreshed successfully" });
        }
        catch (error) {
